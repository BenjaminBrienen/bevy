...
/// fn my_system(mut reader: EventMutator<'_, '_, MyEvent>) {
...
impl<'w, 's, E: Event> EventMutator<'w, 's, E> {
    ...
    /// schedule.add_systems(|mut events: EventMutator<'_, '_, MyEvent>, counter: Res<'_, Counter>| {
    ...
    ///
    /// fn play_collision_sound(mut events: EventMutator<'_, '_, CollisionEvent>) {
    ///     if !events.is_empty() {
    ///         events.clear();
    ///         // Play a sound
    ///     }
